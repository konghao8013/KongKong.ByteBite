...
- 商家从订单管理或其他订单上下文查看消息时跳转到 `/merchant/messages?orderId=...&customerId=...&deviceId=...&returnTo=...`，消息模块根据订单自动创建或打开会话；如会话尚未存在，使用订单里的顾客身份创建会话，避免商家主动发起消息时出现 404；会话页返回时优先回到来源页面。
