...
| 订单会话 | CUST-021, MER-015 | `ConversationsController.GetOrCreateByOrder`; `GetByStore`; `GetByCustomer`; `GetMessages`; `SendMessage`; `MarkRead` | `ConversationService.GetOrCreateByOrderAsync`; `GetByStoreAsync`; `GetByCustomerAsync`; `GetMessagesAsync`; `SendMessageAsync`; `MarkReadAsync` | `Conversation`, `ConversationMessage`, `ConversationHub` | `docs/specs/spec-customer.md`; `docs/specs/spec-order-pickup.md` |
...
| 系统能力 | SYS-001, SYS-002, SYS-003, SYS-004, SYS-005, SYS-007 | `Program.cs`; `GlobalFilters.cs`; `SeedData`; `OrderHub`; `StoreHub`; `ConversationHub` | Seeders and notification services | `ApiResponse`, `ObjectResult.DeclaredType` wrapping fix, EF migrations, SignalR hubs, default configs; `ApiSurfaceBuildTests`, `ApiResponseTests`, `ApiResponseWrapperFilterTests`, current unit tests | `docs/architecture/overview.md`; `docs/specs/spec-testing.md`; `docs/data/seed-data.md` |
...
| 订单会话 | CUST-021, MER-015 | `conversationApi.startByOrder`; `getByStore`; `getByCustomer`; `getMessages`; `sendMessage`; `markRead` | `OrderDetail.vue`; `Orders.vue` | `useSignalR`; `models/conversation` | `docs/specs/spec-customer.md`; `docs/specs/spec-order-pickup.md` |
...
| 全局前端基础 | SYS-001, SYS-004, SYS-005 | `web/src/api/index.ts` response unwrap; API modules under `web/src/api/modules` | `web/src/router/index.ts`; shared layouts | `useSignalR`; `useDeviceId`; common components | `docs/frontend/overview.md`; `docs/frontend/components.md`; `docs/specs/spec-testing.md` |
