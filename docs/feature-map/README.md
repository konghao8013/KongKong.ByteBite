...
| 优惠活动 | CUST-008, MER-013 | `DiscountRulesController` CRUD/list/active | `DiscountRuleService.CreateAsync`; `UpdateAsync`; `DeleteAsync`; `GetByStoreIdAsync`; `GetActiveByStoreIdAsync`; `ValidateAsync`; `OrderService.CalculateBestDiscountAsync`; `SeedData.StoreMenuSeeder/RepairDemoDiscountRulesAsync` | `DiscountRule`, `DiscountEffectStat`; `OrderServiceDiscountTests`, `DiscountRuleServiceTests`, `DiscountRulesIntegrationTests` | `docs/contracts/merchant.md`; `docs/specs/spec-discount.md`; `docs/data/seed-data.md` |
...
| 订单会话 | CUST-021, MER-015 | `ConversationsController.GetOrCreateByOrder`; `GetOrCreateByOrderForMerchant`; `GetByStore`; `GetByCustomer`; `GetStoreUnreadCount`; `GetCustomerUnreadCount`; `GetMessages`; `SendMessage`; `MarkRead` | `ConversationService.GetOrCreateByOrderAsync`; `GetOrCreateByOrderForMerchantAsync`; `GetByStoreAsync`; `GetByCustomerAsync`; `GetUnreadCountForStoreAsync`; `GetUnreadCountForCustomerAsync`; `GetMessagesAsync`; `SendMessageAsync`; `MarkReadAsync` | `Conversation`, `ConversationMessage`, `ConversationHub` 支持 conversation/store/customer/device 订阅组且顾客 ID 优先；SignalR 推送使用扁平 DTO；`StoreHub` 兼容商家订单页消息提示；`ConversationServiceTests` 覆盖商家按订单创建会话 | `docs/specs/spec-customer.md`; `docs/specs/spec-order-pickup.md`; `docs/contracts/order.md` |
...
| 文件上传 | SYS-006 | `FilesController.Upload` `POST /api/files/upload` | Local upload handling in controller; 5MB file limit with multipart request headroom | `wwwroot/uploads`, static files/default index/SPA fallback in `Program.cs`; `FilesControllerTests` 覆盖 5MB 边界 | `docs/specs/spec-frontend.md`; `docs/frontend/components.md` |
...
| 订单会话 | CUST-021, MER-015 | `conversationApi.startByOrder`; `startByOrderForMerchant`; `getByStore`; `getByCustomer`; `getStoreUnreadCount`; `getCustomerUnreadCount`; `getMessages`; `sendMessage`; `markRead` | `OrderDetail.vue` 联系商家跳转 `/A/:code/messages?orderId=...&returnTo=...`；`Orders.vue` 消息入口跳转 `/merchant/messages?orderId=...&customerId=...&deviceId=...&returnTo=...`；`customer/Messages.vue` 和 `merchant/Messages.vue` 先列表后会话详情，列表保留底部模块菜单、详情隐藏底部菜单，支持按订单自动创建/打开会话并返回来源页；`CustomerLayout.vue`; `MerchantLayout.vue` | `useSignalR` 重连回调；`useCustomerIdentity` 游客顾客 ID；`useConversationStore`; `models/conversation`; 商家主动发起消息复用通用订单会话接口并携带订单顾客身份；仅中间消息区滚动，消息加载/发送/接收后滚动到底，商家回复输入区吸附底部 | `docs/specs/spec-customer.md`; `docs/specs/spec-order-pickup.md`; `docs/frontend/overview.md` |
