...
3. Read `docs/feature-map/README.md` to locate the related backend methods, frontend methods, pages, docs, and tests.
4. If the user adds a new requirement, add or update checklist rows first with status `未实现` unless there is verified existing implementation.
5. When implementing a feature, update the matching checklist row in the same turn:
...
6. Update `docs/feature-map/README.md` when the implementation adds, removes, renames, or moves backend methods, endpoints, frontend API methods, pages, stores, composables, model types, docs, or test evidence.
7. Keep feature IDs stable. Do not delete rows for removed or deferred scope; mark the reason in `待补齐/备注`.
8. Add concise evidence in `当前依据`, such as controller/service/page/test filenames.
9. Update `最后盘点日期` when doing a broad checklist review.
10. Treat every `部分实现` and `未实现` checklist item as implementation work to finish in the current scope unless it depends on paid third-party services.
...
- Use `docs/skills/bytebite-feature-locator/SKILL.md` when the task requires finding or updating feature implementation points.
